...
$kat_katalog = (isset($_POST['kat_katalog'])) ? htmlentities($_POST['kat_katalog']) : " ";
    ...
    if ($statusUpload == 0) {
        ...
        if (mysqli_num_rows($select) > 0) {
            ...
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
                $query = mysqli_query($conn, "INSERT INTO tb_katalog (foto, nama_desain, keterangan, kategori) VALUES ('" . $kode_rand . $_FILES['foto']['name'] . "','$nama_desain', '$keterangan', '$kat_katalog')");
                if ($query) {
